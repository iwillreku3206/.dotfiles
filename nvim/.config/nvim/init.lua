...
require("lazy").setup({
	...
	{ -- Highlight, edit, and navigate code
		...
		build = function()
			...
			local current_dir = debug.getinfo(1).source:sub(2):match("(.*[/\\])")
			print("current_dir: ", current_dir)
			local ftplugin_path = vim.fs.joinpath(current_dir, "ftplugin")
			if vim.fn.isdirectory(ftplugin_path) == 0 then
				vim.fn.mkdir(ftplugin_path)
			end
			...
			for _, value in ipairs(list) do
				...
				local ftpath = vim.fs.joinpath(ftplugin_path, value .. ".lua")

				local file = io.open(ftpath, "w")
				if file then
					file:write([[-- autogenerated in init.lua
...
vim.bo.indentexpr = "v:lua.require'nvim-treesitter'.indentexpr()"
					...
					error("Error opening file: " .. ftpath)
				end
				::continue::
			end
